...
"$Id: getqueue.c,v 5.15 2000/04/14 20:39:52 papowell Exp papowell $";
		...
		int only_pr_and_move )
{
	...
	for( df_count = 0; df_count < directory_files.count; ++df_count ){
		...
		if (s == hf_name+6) {
			/* we are happy, at least it is not too long */
		} else if ( Is_IP_addr( hf_name+6 ) || s < hf_name+9){
			/* we assume 3 characters */
			s = hf_name+6;
		} else {
			s = hf_name+9;
		}
	...
	for( i = 0; i < cf_line_list->count; ++i ){
		...
		if( islower(c) ){
			...
			if( stat(s, &statb) == 0 ){
				double size;
				size = statb.st_size;
				DEBUG4("Get_datafile_info: '%s' - size %0.0f", s, size );
 ...
 *  returns 1 if string has format nn.nn.nn.nn
 *          0 otherwise
 ***************************************************************************/
	...
	char *xlate_format )
{
	...
	for( linecount = 0; linecount < job->datafiles.count; ++linecount ){
		...
		if( !(s = Find_casekey_str_value(&outfiles,transfername,Value_sep)) ){
			...
			if( c > 'Z' ) c += 'a' - 'Z' - 1;
			++count;
			SNPRINTF(prefix,sizeof(prefix),"df%c",c);

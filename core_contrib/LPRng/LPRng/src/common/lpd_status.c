...
"$Id: lpd_status.c,v 5.9 2000/04/14 20:40:00 papowell Exp papowell $";
...
" Rank   Owner/ID                  Class Job Files                 Size Time"
	...
	for( count = 0;
		...
		if( displayformat == REQ_DLONG ){
			...
			if( Find_flag_value(&job.info, PRINTABLE, Value_sep ) ){
				++matches;
			}
		}
	...
	if( (s = Clsses(&Spool_control)) ){
		len = strlen(msg);
		SNPRINTF( msg+len, sizeof(msg)-len, _(" Classes: %s\n"), s );
	}
	if( msg[0] ){
		if( Write_fd_str( *sock, msg ) < 0 ) cleanup(0);
	}

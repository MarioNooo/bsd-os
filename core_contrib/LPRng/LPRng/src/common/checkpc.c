...
"$Id: checkpc.c,v 5.9 2000/04/14 20:39:49 papowell Exp papowell $";
...

	{
		...
		if( error[0] ){
			...
			if( (p = strchr(error,':')) ){
				*p++ = 0; } else { p=error; }
			Warnmsg(
				"%s: server printcap entry ':%s' and ':%s' are conflicting",
				Printer_DYN, error, p );
		}
	}
...
void Clean_log( int trunc, char  *dpath, char *logfile )
{
	...
	while( logfile && *logfile ){
		...
		k = (trunc >= 0 && len > trunc);
		sx = (k)?"needs":"no";
		SNPRINTF(buffer,sizeof(buffer)," to %dK", trunc );

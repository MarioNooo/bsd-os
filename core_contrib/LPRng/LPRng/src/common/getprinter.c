...
"$Id: getprinter.c,v 5.8 2000/04/18 07:39:48 papowell Exp papowell $";
	...
	if( (s = safestrchr( Printer_DYN, '@' ))  ){
		...
		*s++ = 0;
		...
		if( !Is_server && Force_localhost_DYN ){
			 ...
			 */
			if( Lp_device_DYN && (s = safestrchr( Lp_device_DYN, '@' )) ){
			...
			if( RemoteHost_DYN && report_conflict ){
			...
			DEBUG2("Fix_Rm_Rp_info: checking conflict :lp='%s', :rp='%s', '%s'",
				Lp_device_DYN, RemoteHost_DYN, report_conflict );
			Set_DYN(&RemoteHost_DYN,0);
			Set_DYN(&RemotePrinter_DYN,0);
		} else {
			if( RemoteHost_DYN == 0 ){

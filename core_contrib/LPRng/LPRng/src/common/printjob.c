...
"$Id: printjob.c,v 5.9 2000/04/14 20:40:04 papowell Exp papowell $";
...
int Print_job( int output, struct job *job, int timeout )
{
	...
	of_fd[0] = of_fd[1] = of_error[0] = of_error[1] = tempfd = fd = -1;
	...
	if( OF_Filter_DYN ){
        ...
        if( (of_pid = Make_passthrough( OF_Filter_DYN, s,&files, job, 0 ))<0){
            Errorcode = JFAIL;
            logerr(LOG_INFO,"Print_job: could not create OF process");
			goto exit;
        }
		files.count = 0;
		...
		if( (close( of_fd[0] ) == -1 ) ){
			Errorcode = JFAIL;
			logerr( LOG_INFO,"Print_job: close(%d) failed", of_fd[0]);
			goto exit;
		}
		of_fd[0] = -1;
		if( (close( of_error[1] ) == -1 ) ){
			Errorcode = JFAIL;
			logerr( LOG_INFO,"Print_job: close(%d) failed", of_error[1]);
			goto exit;
		}
		of_error[1] = -1;
	...
	for( count = 0; count < job->datafiles.count; ++count ){
		...
		case 'p': case 'f': case 'l':
			...
			if( c == 'p' ){
				...
				if( Pr_program_DYN == 0 ){
					...
					Errorcode = JABORT;
					logmsg( LOG_ERR, "Print_job: no '-p' formatter for '%s'",
						c, id );
					goto exit;
				}
			}
		...
		default:
			...
			if( filter == 0 ){
				...
				Errorcode = JABORT;
				logmsg( LOG_ERR, "Print_job: cannot find '%s' filter",
					filter_name );
				goto exit;
			}
		}
	...
	if(DEBUGL5){
		...
		for( i = 0; i < 20; ++i ){
			...
			if( filter ){
				...
				SNPRINTF(buffer,sizeof(buffer),", %s filter '%s'", filter_title, msg );
			}
			SETSTATUS(job)"printing data file '%s', size %0.0f%s",
				transfername, (double)statb.st_size, filter?buffer:"" );
			...
			if( 'p' == c ){
				...
				if( lseek(fd,0,SEEK_SET) < 0 ){
					Errorcode = JFAIL;
					logerr( LOG_INFO,"Print_job: fseek(%d) failed", fd);
					goto exit;
				}
			}
			if( filter ){
				...
				if( (close(if_error[0]) == -1 ) ){
					Errorcode = JFAIL;
					logerr( LOG_INFO,"Print_job: close(%d) failed",
						if_error[1]);
					goto exit;
				}
			} else {
				...
				while( (n = read(fd,buffer,sizeof(buffer))) > 0 ){
					if( Write_fd_len(output, buffer, n ) < 0 ){
						Errorcode = JFAIL;
						logerr(LOG_INFO,"Print_job: write to output failed");
						goto exit;
					}
				}
				if( (close(fd) == -1 ) ){
					Errorcode = JFAIL;
					logerr( LOG_INFO,"Print_job: close(%d) failed", fd);
					goto exit;
				}
 ...
 exit:
	...
	if( of_fd[0] != -1 ) close(of_fd[0]); of_fd[0] = -1;
	if( of_fd[1] != -1 ) close(of_fd[1]); of_fd[1] = -1;
	if( of_error[0] != -1 ) close(of_error[0]); of_error[0] = -1;
	if( of_error[1] != -1 ) close(of_error[1]); of_error[1] = -1;

...
"$Id: linelist.c,v 5.20 2000/04/14 20:39:54 papowell Exp papowell $";
	...
	const char *str )
{
	...
	for( count = 0; count < filters->count; ++count ){
		...
		while( (n = plp_waitpid(pid,&status,0)) != pid ){
			...
			Errorcode = JABORT;
			logerr_die( LOG_ERR, "Filterprintcap: waitpid(%d) failed", pid);
		} 
...
int Make_temp_fd( char **temppath )
{
	...
	Init_tempfile();
	SNPRINTF(buffer,sizeof(buffer),"%02dXXXXXX", Tempfiles.count );
	...
	if( fchmod(tempfd,(Is_server?Spool_file_perms_DYN:0) | 0600 ) == -1 ){
		Errorcode = JFAIL;
		logerr_die(LOG_INFO,"Make_temp: chmod '%s' to 0%o failed ",
	...
	struct job *job, struct line_list *env_init )
{
	...
	while( cmd.count ){
		if( !safestrncmp((s = cmd.list[0]),"$-", 2)
			||	!safestrncmp((s = cmd.list[0]),"-$", 2) ){
			noopts = 1;
			memmove( cmd.list[0], cmd.list[0]+2, strlen(cmd.list[0]+2)+1);
			if( cval(cmd.list[0]) ){
				continue;
			}
		} else if( !safestrncasecmp((s = cmd.list[0]),"root", 4) ){
			root = 1;
			memmove( cmd.list[0], cmd.list[0]+4, strlen(cmd.list[0]+4)+1);
			if( cval(cmd.list[0]) ){
				continue;
			}
		} else {
	...
	int *pgp_exit_code, int *not_a_ciphertext )
{
	...
	*pgp_exit_code = *not_a_ciphertext = 0;
	...
	if( pipe(error_fd) == -1 ){
		Errorcode = JFAIL;
		logerr_die(LOG_INFO, "Pgp_decode: pipe failed - '%s'" );
	}
	Check_max(&files,10);
	...
	int *pgp_exit_code )
{
	...
	*pgp_exit_code = 0;
	...
	if( pipe(error_fd) == -1 ){
		Errorcode = JFAIL;
		logerr_die(LOG_INFO, "Pgp_encode: pipe failed - '%s'" );
	}
	Check_max(&files,10);
	files.count = 0;
	files.list[files.count++] = Cast_int_to_voidstar(0);

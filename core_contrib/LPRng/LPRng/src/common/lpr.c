...
"$Id: lpr.c,v 5.10 2000/04/14 20:40:01 papowell Exp papowell $";
...
int main(int argc, char *argv[], char *envp[])
{
	...
	if( Errorcode ){
		...
		if( !Verbose ){
			...
			if( (s = Find_str_value(&prjob.info,ERROR,Value_sep)) ){
				SNPRINTF(buffer,sizeof(buffer), "job failed - %s\n",s );
			}
	...
	if( LP_mode && !Silent ){
		...
		if( id ){
			SNPRINTF( msg,sizeof(msg)-1,"request id is %s\n", id );
		} else {
			n = Find_decimal_value(&prjob.info,NUMBER,Value_sep);
			SNPRINTF( msg,sizeof(msg)-1,"request id is %d\n", n );
		}
		Write_fd_str(1, msg );
	}
...
void usage(void)
{
	...
	fprintf( stderr, "LPRng Version %s\n", PATCHLEVEL );
	exit(1);
}
...
int Make_job( struct job *job )
{
	...
	SNPRINTF(nstr,sizeof(nstr),"%c",Priority);
	...
	SNPRINTF(nstr,sizeof(nstr),"%c",Format);
...
double Copy_stdin( struct job *job )
{
	...
	Set_str_value(lp,TRANSFERNAME,tempfile);
	Set_flag_value(lp,COPIES,1);
	SNPRINTF(buffer,sizeof(buffer),"%c",Format);

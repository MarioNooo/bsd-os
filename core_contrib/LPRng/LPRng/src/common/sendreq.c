...
"$Id: sendreq.c,v 5.6 2000/04/14 20:40:09 papowell Exp papowell $";
	...
	)
{
	...
	if( security ){
		DEBUG1("Send_request: security name '%s', tag '%s', connect %d, send %d, receive %d",
		security->name, security->config_tag, (int)security->connect, (int)security->send,
		(int)security->receive );
	}
	if( errormsg[0] ){
		goto error;
	}
	...
	if( Remote_support_DYN ){
		if( safestrchr( Remote_support_DYN, class ) == 0 ){
			...
			if( !Is_server ){
				SNPRINTF(errormsg,sizeof(errormsg),_("no network support for request"));
			} else {
				status = 0;
			}
			goto error;
		...
		if( format == REQ_VERBOSE
			...
			if( !Is_server ){
				SNPRINTF(errormsg,sizeof(errormsg),_("no network support for verbose status"));
			} else {
				status = 0;
			}
			goto error;

...
"$Id: lpd_rcvjob.c,v 5.14 2000/04/14 20:39:58 papowell Exp papowell $";
...
int Receive_job( int *sock, char *input )
{
	...
	if( Setup_printer( info.list[0], error, errlen ) ){
		SNPRINTF( error, errlen, _("%s: no spool queue"), Printer_DYN );
		goto error;
	}
...
int Receive_block_job( int *sock, char *input )
{
	...
	if( Setup_printer( l.list[0], error, errlen-4 ) ){
		SNPRINTF( error, errlen-4, _("%s: no spool queue"), Printer_DYN );
		goto error;
	}
	...
	char *error, int errlen, char *auth_id )
{
	...
	/* now we rename the data files - if we get killed at this point we leave
	   junk in the spool queue - we will have to live with this.
     */
 ...
 error:
	if( status ){
		if( error[0] ){
			logmsg( LOG_INFO, "Check_for_missing files: %s", error);
		}
...
int Get_route( struct job *job, char *error, int errlen )
{
	...
	if( pipe(out) == -1 ){
		SNPRINTF(error,errlen,"Get_route: pipe failed '%s'",
			Errormsg(errno) );
		errorcode = 1;
		goto error;
	}
	...
	if( pid < 0 ){
		SNPRINTF(error,errlen,"Get_route: filter creation failed");
		errorcode = 1;
		goto error;
	}
	close(out[1]);

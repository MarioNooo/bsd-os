 ...
 **************************************************************************/
/**** HEADER *****/
 static char *const _id = "$Id: checkcode.c,v 1.4 2000/04/14 21:33:47 papowell Exp papowell $";
...
/*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 ...
 static char * lookup(code_t *tab, int target)
{
 ...
 static char * pjl_message(char *code_str)
{
	...
	if (PJLC_GROUP(code) == 11
	 || PJLC_GROUP(code) == 41) {
		...
		SNPRINTF(buf, sizeof(buf), "Load `%s' into the `%s' tray",

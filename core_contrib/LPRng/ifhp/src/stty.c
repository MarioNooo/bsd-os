 ...
 **************************************************************************/
/**** HEADER *****/
 static char *const _id = "$Id: stty.c,v 1.3 1999/12/17 02:04:59 papowell Exp papowell $";
...
{
	int i, localmode, linedisc, argcount;
	char *s, *arg, *parm = 0;
	struct line_list l;

	Init_line_list( &l );
	Split( &l, Stty_command, Whitespace, 0, 0, 0, 1, 0 );

	DEBUG4("Do_stty: using SGTTYB, fd %d", fd );
	if( ioctl( fd, TIOCGETP, &mode) < 0
		|| ioctl( fd, TIOCGETC, &termctrl) < 0
		|| ioctl( fd, TIOCLGET, &localmode) < 0
		|| ioctl( fd, TIOCGLTC, &linectrl) < 0 ){
		Errorcode = JABORT;
		logerr_die( "cannot get tty parameters");
	}
	DEBUG4("Do_stty: before mode 0x%x, lmode 0x%x, speed 0x%x",
			 mode.sg_flags, localmode, mode.sg_ispeed);
	for( argcount = 0; argcount < l.count; ++argcount ){
		arg = l.list[argcount];
		for( i = 0; modes[i].string && strcasecmp( modes[i].string, arg); i++);
		if( modes[i].string ){
			DEBUG4("Do_stty: modes %s, mc 0x%x ms 0x%x lc 0x%x ls 0x%x",
		...
		if( special[i].name) {
			if( ++argcount < l.count ){
				parm = l.list[argcount];
			} else {
				Errorcode = JABORT;
				fatal(  "Do_stty: missing parameter for %s\n", arg);
			}
			if( parm[0] == '^') {
				if( parm[1] == '?') {
					*special[i].cp = 0177;
				} else {
					*special[i].cp = 037 & parm[1];
				}
			} else {
				*special[i].cp = parm[0];
			}
	...
	if( ioctl( fd, TIOCSETN, &mode) < 0
		...
		Errorcode = JABORT;
		logerr_die( "cannot set tty parameters");
	}
	Free_line_list( &l );
}
#endif
 ...
 void Do_stty( int fd, char *Stty_command )
{
	...
	DEBUG4("Do_stty: using TERMIO, fd %d", fd );
	...
	for( argcount = 0; argcount < l.count; ++argcount ){
		...
		if( tmodes[i].string) {
			DEBUG4("Do_stty: modes %s, ic 0x%x is 0x%x oc 0x%x os 0x%x cc 0x%x cs 0x%x lc 0x%x ls 0x%x",
 ...
 void Do_stty( int fd, char *Stty_command )
{
	...
	DEBUG4("Do_stty: using TERMIOS, fd %d", fd );
	if( tcgetattr( fd, &t_dat) < 0 ){
		Errorcode = JABORT;
		logerr_die("cannot get tty parameters");
	}
	DEBUG4("Do_stty: before iflag 0x%x, oflag 0x%x, cflag 0x%x lflag 0x%x",
	...
	for( argcount = 0; argcount < l.count; ++argcount ){
		...
		if( bauds[i].string ){
			...
			DEBUG4("Do_stty: using cfsetispeed/cfsetospeed, speed %s, spd %d",
				arg, bauds[i].speed );
			/* POSIX baudrate manipulation */
			cfsetispeed( &t_dat, bauds[i].speed);
			cfsetospeed( &t_dat, bauds[i].speed);
#else
			DEBUG4("Do_stty: setting tdat.c_cflag");
			t_dat.c_cflag &= ~CBAUD;
		...
		if( c_i_dat[i].name ){
			DEBUG4("Do_stty: c_iflag %s, ms 0x%x mc 0x%x",
		...
		if( c_o_dat[i].name ){
			DEBUG4("Do_stty: c_oflag %s, ms 0x%x mc 0x%x",
		...
		if( c_c_dat[i].name ){
			DEBUG4("Do_stty: c_cflag %s, ms 0x%x mc 0x%x",
		...
		if( c_l_dat[i].name ){
			DEBUG4("Do_stty: c_lflag %s, ms 0x%x mc 0x%x",
	...
	if( (t_dat.c_lflag & ICANON) == 0 ){
		...
		DEBUG4("setting port to read/write with unbuffered reads( MIN=1, TIME=0)");
		t_dat.c_cc[VMIN] = 1;
		t_dat.c_cc[VTIME] = 0;
	}
	DEBUG4("Do_stty: after iflag 0x%x, oflag 0x%x, cflag 0x%x lflag 0x%x",
			 t_dat.c_iflag, t_dat.c_oflag, t_dat.c_cflag, t_dat.c_lflag);

...
/*
 *    valid                                new state if
 *      in    cmd    params:               command:
 *    state   name   min max  function     fails  succeeds
 *    -----   -----  --- ---  --------     -----  -----    */
    ...
    { auth1,  "auth", 0,  2,  pop_auth,   {auth1, auth1} },
#ifdef RPOP
    ...
    { trans,  "mdef", 1,  1,  pop_mdef,   {trans, trans} },
    { trans,  "stat", 0,  0,  pop_stat,   {trans, trans} },
    { trans,  "list", 0,  2,  pop_list,   {trans, trans} },
    ...
    { trans,  "top",  2,  3,  pop_send,   {trans, trans} },
    ...
    { trans,  "epop", 0,  0,  pop_epop,   {trans, trans} },
                                    ...
                                    received from the client */
{
     ...
     */
    if ( DEBUGGING && p->debug )
        strncpy(buf, mp, sizeof(buf));

    /* 
     * Parse the message into the parameter array
     */
    p->parm_count = pop_parse ( p, mp );
    if ( p->parm_count < 0)\
        return ( NULL );
    ...
    if ( DEBUGGING && p->debug ) {
        if ( strcmp ( p->pop_command, "pass" ) == 0 )
            pop_log ( p, POP_DEBUG, HERE, "Received: \"%s xxxxxxxxx\"",
                      p->pop_command );
        else {
            int len = strlen(buf);
            while ( len > 0 &&
            ...
            buf [ len ] = '\0';
            pop_log ( p, POP_DEBUG, HERE, "Received (%d): \"%s\"", len, buf );
        }
    }
        ...
        if ( strcmp ( s->command, p->pop_command ) == 0
            ...
            if ( p->parm_count < s->min_parms ) {
                pop_msg ( p, POP_FAILURE, HERE,
                          "Too few arguments for the %.100s command.",
                          p->pop_command );
            ...
            if ( p->parm_count > s->max_parms ) {
                pop_msg ( p, POP_FAILURE, HERE,
                          "Too many arguments for the %.100s command.",
                          p->pop_command );
                return ( (state_table *) 0 );
            }
     ...
     * truncate the unknown command; to avoid buffer overruns
     */
    pop_msg ( p, POP_FAILURE, HERE, "Unknown command: \"%.100s\".",
              p->pop_command );
    return ( (state_table *) 0 );
}

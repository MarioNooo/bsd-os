 ...
 *
 *     11/27/01  [rcg]
 *              - Worked around a bug (since fixed) in OpenBSD by
 *                setting file descriptor to be blocking *before*
 *                we fork, and ignoring what fcntl() says it is.
 *
 *     04/03/01  [rcg]
...
main ( int argc, char *argv[] )
{
    ...
    rslt     = fcntl ( sockfd, F_SETFL, fd_flags | O_NONBLOCK );
    if ( rslt == -1 )
...
motherforker ( int newsockfd, int sockfd )
{
    ...
    /*
     * Make sure we pass a blocking socket to Qpopper.  Do this before we
     * fork, and even if fcntl() insists the socket is already blocking,
     * because of a bug in older versions of OpenBSD.
     */

/*	BSDI c-typeck.c,v 1.3 2003/03/04 02:02:52 polk Exp	*/
...
#ifdef __bsdi__ /* not the right ifdef, want "if target has unsigned ptrs" */
      /* When the result has more bits than the input, insert an extra
	 conversion to force unsigned expansion, i.e., pointers are always
	 unsigned.  (There must be a better way to do this!)  */
      if (TREE_CODE (type) == INTEGER_TYPE
	  && TREE_CODE (otype) == POINTER_TYPE
	  && TYPE_PRECISION (type) > TYPE_PRECISION (otype))
	value = convert (type_for_size (POINTER_SIZE, 1), expr);
#endif
     ...
     int parmnum;
{
  ...
  /* Conversions among pointers */
  ...
  else if (codel == INTEGER_TYPE && coder == POINTER_TYPE)
    {
	 ...
	 conversion to force unsigned expansion, i.e., pointers are always
	 unsigned.  (There must be a better way to do this!)  */
      if (TYPE_PRECISION (type) > TYPE_PRECISION (rhstype))
	rhs = convert (type_for_size (POINTER_SIZE, 1), rhs);
#endif
      return convert (type, rhs);
    }

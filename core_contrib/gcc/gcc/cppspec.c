/*	BSDI cppspec.c,v 1.2 2003/01/31 08:07:15 torek Exp	*/
     ...
     int *in_added_libraries ATTRIBUTE_UNUSED;
{
  ...
  for (i = 1; i < argc; i++)
	...
	{
	  seen_input++;
#ifndef CPP_TAKES_MULTIPLE_ARGUMENTS
	  if (seen_input == 3)
	    {
...
#else
  new_argc = argc + need_E + need_no_gcc + read_stdin
    + (need_fixups ? (lang_c_here + lang_S_here) * 2 : 0);
#endif
  ...
  for (i = 1; i < argc; i++, j++)
      ...
      if (quote == 1)
	{
      ...
      if (argv[i][0] == '-')
	{
	  if (argv[i][2] == '\0' && SWITCH_TAKES_ARG (argv[i][1]))
	    quote = 1;
	  if (WORD_SWITCH_TAKES_ARG (&argv[i][1]))
	    quote = 1;
	}
      else if (need_fixups)
	{

/*	BSDI ggc-page.c,v 1.2 2003/01/31 08:09:18 torek Exp	*/
...
#ifdef __bsdi__
/* XXX -- we have mmap but we need to work on the kernel implementation.
   Using mmap to allocate anonymous memory eats up limited resources
   (kernel "vm_map_entry" data structures) while ordinary malloc/sbrk
   does not.  The default per-process limit (sysctl vm.maxmapentries)
   cause the this code to die after allocating only about 11 MB.  The
   vm_map_entry data structures are supposed to be compactable, but
   until the kernel does something different/better, we just disable
   use of mmap for the garbage collector.  */
#undef USING_MMAP
#endif

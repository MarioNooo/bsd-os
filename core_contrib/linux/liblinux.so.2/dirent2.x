/*	BSDI dirent2.x,v 1.3 2000/12/08 03:45:19 donn Exp	*/

/*
 * Support for inline system calls for directories.
 * Glibc doesn't export getdents(), but it does use it internally.
 */
...
#define	DIR_SHIM \
	(sizeof (struct linux_kernel_dirent) - sizeof (struct dirent))
%}
...
struct dirbuf {
	...
	out(n, f, len) {
		struct dirent *dp;
		struct linux_kernel_dirent *fdp;
		int size = 0;

		for (dp = (struct dirent *)n->buf;
		    size < n->size;
		    dp = (struct dirent *)((char *)dp + dp->d_reclen)) {
			fdp = (struct linux_kernel_dirent *)&f->linux_buf[size];
			kernel_dirent_out(dp, fdp, dp->d_reclen + DIR_SHIM);
...
int __kernel_getdents(int fd, struct dirbuf *dbuf, size_t length_dbuf)
{
	char *buf;
	struct dirent *dp;
	 ...
	 */
	do {
		if (__bsdi_error(r = __bsdi_syscall(SYS_getdirentries, fd, buf,
		    n_size, &off)))
			return (r);

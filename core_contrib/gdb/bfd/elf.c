...
static boolean elfcore_grok_bsdi_note PARAMS ((bfd *, Elf_Internal_Note *));
...
/* BSD/OS specific core note handling.  The notes are vaguely similar
   to SVr4 core notes, but not sufficiently similar that we can share code.

   NT_PRSTATUS: general registers plus terminating signal
   NT_FPREGSET: floating point state (plus pcb, on some architectures)
   NT_PRPSINFO: procinfo structure, including PID

   FIXME: This code assumes a 32-bit word size and a fixed kinfo_proc layout.
   FIXME: The OS-dependent code belongs in an OS-dependent source file.  */

#define	KPROC_PID_OFFSET	0x3c

static boolean
elfcore_grok_bsdi_note (abfd, note)
     bfd *abfd;
     Elf_Internal_Note *note;
{
  switch (note->type)
	...
	return false;
      elf_tdata (abfd)->core_signal
	= bfd_get_32 (abfd, note->descdata + note->descsz - 4);
      note->descsz -= 4;
      return elfcore_make_note_pseudosection (abfd, ".reg", note);
    ...
    case NT_PRPSINFO:
      if (note->descsz < KPROC_PID_OFFSET + 4)
	break;
      elf_tdata (abfd)->core_pid
	= bfd_get_32 (abfd, note->descdata + KPROC_PID_OFFSET);
      return true;
  ...
  while (p < buf + size)
    {
      ...
      else if (strncmp (in.namedata, "BSD/OS", 7) == 0)
	{
	  if (! elfcore_grok_bsdi_note (abfd, &in))
	    goto error;
	}
      else
        {

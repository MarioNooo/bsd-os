...

struct rs6000_abi_handler *rs6000_abi_handler_list = NULL;

void
rs6000_gdbarch_register_os_abi (int abi, int cookie,
     boolean (*check_abi) (struct rs6000_abi_handler *, bfd *),
     void (*init_abi) (struct gdbarch_info, struct gdbarch *))
{
  struct rs6000_abi_handler **handlerp;

  for (handlerp = &rs6000_abi_handler_list; *handlerp != NULL;
       handlerp = &(*handlerp)->next)
    if ((*handlerp)->abi == abi)
      {
	internal_error
	  (__FILE__, __LINE__,
	   "rs6000_gdbarch_register_os_abi: A handler for this ABI variant (%d)"
  ...
  *handlerp
    = (struct rs6000_abi_handler *) xmalloc (sizeof (struct rs6000_abi_handler);
  (*handlerp)->next = NULL;
  (*handlerp)->abi = abi;
  (*handlerp)->cookie = cookie;
  (*handlerp)->check_abi = check_abi;
...
static boolean
rs6000_check_gnu_abi (struct rs6000_abi_handler *handler, bfd *abfd)
{
...
static void
rs6000_init_abi_linux (struct gdbarch_info info, struct gdbarch *gdbarch)
{
  set_gdbarch_frameless_function_invocation
    (gdbarch, ppc_linux_frameless_function_invocation);
...
rs6000_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  ...
  if (info.abfd != NULL
    ...
    {
      osabi = ELFOSABI_NONE;
      for (abi_handler = rs6000_abi_handler_list; abi_handler != NULL;
	   abi_handler = abi_handler->next)
	if (abi_handler->check_abi (handler, info.abfd))
	  {
	    osabi = abi_handler->abi;
	    break;
	  }
      if (osabi == ELFOSABI_NONE)
	osabi = elf_elfheader (info.abfd)->e_ident[EI_OSABI];
    }
  else
    osabi = -1;
...
_initialize_rs6000_tdep (void)
{
  ...
  rs6000__gdbarch_register_os_abi (ELFOSABI_LINUX, 0, rs6000_check_gnu_abi,
				   rs6000_init_abi_linux);
  rs6000__gdbarch_register_os_abi (ELFOSABI_HURD, 1, rs6000_check_gnu_abi,
				   NULL);
  rs6000__gdbarch_register_os_abi (ELFOSABI_SOLARIS, 2, rs6000_check_gnu_abi,
				   NULL);

...
/* Immediately after a function call, return the saved pc.
   Find the saved return pc even in frameless library functions.
   We assume that such functions start with a conventional prologue,
   except for the creation of the stack frame.  */

CORE_ADDR
i386_saved_pc_after_call (struct frame_info *fi)
{
  CORE_ADDR sp = read_register (SP_REGNUM);
  CORE_ADDR end_pc = read_pc ();
  CORE_ADDR pc = get_pc_function_start (end_pc);
  char buf[4];
  unsigned int imm;
  ...
  while (codestream_tell () < end_pc)
	...
	case 0x57:	/* pushl %edi */
	  sp += 4;
	  break;
	case 0x81:	/* subl $N,%esp (32-bit immediate) */
	  /* Mod = 11, opcode = 101, r/m = 100 [ESP] */
	  if (codestream_get () != 0xec)
	    goto out;
	  codestream_read (buf, 4);
	  imm = extract_unsigned_integer (buf, 4);
	  sp += imm;
	  break;
	case 0x83:	/* subl $N,%esp (8-bit immediate) */
	  if (codestream_get () != 0xec)
	    goto out;
	  sp += codestream_get ();
	  break;
...
/* Return true if the given pc points at a real function call.  */

int
i386_is_call_insn (CORE_ADDR pc)
{
  char buf[4];
  CORE_ADDR offset;
  ...
  if (codestream_get () != 0xe8)
    return false;
  codestream_read (buf, 4);
  offset = extract_address (buf, 4);
  if (offset == 0)
    /* This is PIC code to recover the current PC.  */
    return false;
  return true;
}
...
void
i386_gdbarch_register_os_abi (int abi, int cookie,
     boolean (*check_abi) (struct i386_abi_handler *, bfd *),
     void (*init_abi) (struct gdbarch_info, struct gdbarch *))
{
  struct i386_abi_handler **handlerp;

  for (handlerp = &i386_abi_handler_list; *handlerp != NULL;
       handlerp = &(*handlerp)->next)
    if ((*handlerp)->abi == abi)
      {
	internal_error
	  (__FILE__, __LINE__,
	   "i386_gdbarch_register_os_abi: A handler for this ABI variant (%d)"
	   " has already been registered", abi);
	/* If user wants to continue, override previous definition.  */
	(*handlerp)->cookie = cookie;
	(*handlerp)->check_abi = check_abi;
	(*handlerp)->init_abi = init_abi;
	return;
      }
...
static void
process_gnu_abi_tag_sections (bfd *abfd, asection *sect, void *obj)
{
...
static boolean
i386_check_gnu_abi (struct i386_abi_handler *handler, bfd *abfd)
{
  if (elf_elfheader (abfd)->e_ident[EI_OSABI] != ELFOSABI_NONE)
    return false;

  bfd_map_over_sections (abfd, process_gnu_abi_tag_sections, &handler);
...
static boolean
i386_check_freebsd_abi (struct i386_abi_handler *handler, bfd *abfd)
{
  if (elf_elfheader (abfd)->e_ident[EI_OSABI] != ELFOSABI_NONE)
    return false;
  return strcmp (&elf_elfheader (abfd)->e_ident[8], "FreeBSD") == 0;
}

struct gdbarch *
i386_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  struct i386_abi_handler *abi_handler = NULL;
  ...
  if (info.abfd != NULL
    ...
    {
      os_ident = ELFOSABI_NONE;
      for (abi_handler = i386_abi_handler_list; abi_handler != NULL;
	   abi_handler = abi_handler->next)
	if (abi_handler->check_abi (abi_handler, info.abfd))
	  {
	    os_ident = abi_handler->abi;
	    break;
	  }
  ...
  if (abi_handler != NULL && abi_handler->init_abi != NULL)
...
_initialize_i386_tdep (void)
{
  ...
  /* These calls really belong in the various i386foo-tdep.c files.  */
  i386_gdbarch_register_os_abi (ELFOSABI_LINUX, 0, i386_check_gnu_abi, NULL);
  i386_gdbarch_register_os_abi (ELFOSABI_HURD, 1, i386_check_gnu_abi, NULL);
  i386_gdbarch_register_os_abi (ELFOSABI_SOLARIS, 2, i386_check_gnu_abi, NULL);
  i386_gdbarch_register_os_abi (ELFOSABI_FREEBSD, 0, i386_check_freebsd_abi,
				NULL);

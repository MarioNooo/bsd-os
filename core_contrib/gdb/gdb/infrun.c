  ...
  if (in_thread_list (inferior_ptid) && in_thread_list (ecs->ptid))
    { /* Perform infrun state context switch: */
      ...
      save_infrun_state (inferior_ptid, prev_pc, 
			 ...
			 step_sp, ecs->remove_breakpoints_on_following_step);
...
/* ??rehrauer: I don't understand the rationale for this code.  If the
  ...
  process_event_stop_test:
	...
	if (ecs->stop_func_start == 0
	    ...
	    && step_range_end == 1)
	  {
	    /* We probably executed a nexti command, and we don't
	       have enough symbol information to tell whether we
	       are still in the same function.  If the last executed
	       instruction was a call, assume that we are in a new
	       function; otherwise, we perform a single step.  */
	    if (! IS_CALL_INSN (prev_pc))
	      {
		stop_step = 1;
		print_stop_reason (END_STEPPING_RANGE, 0);
		stop_stepping (ecs);
...
step_over_function (struct execution_control_state *ecs)
{
  ...
  /* Don't insert a breakpoint at 0 if we have no symbols!  */
  if (sr_sal.pc == 0)
    return;

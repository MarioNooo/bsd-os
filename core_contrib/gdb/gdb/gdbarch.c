...
struct gdbarch
{
  ...
  gdbarch_is_call_insn_ftype *is_call_insn;
};
...
verify_gdbarch (struct gdbarch *gdbarch)
{
  ...
  /* Skip verify of is_call_insn, has predicate */
...
gdbarch_dump (struct gdbarch *gdbarch, struct ui_file *file)
{
  ...
  fprintf_unfiltered (file,
                      "gdbarch_dump: %s # %s\n",
                      "IS_CALL_INSN(pc)",
                      XSTRING (IS_CALL_INSN (pc)));
  if (GDB_MULTI_ARCH)
    fprintf_unfiltered (file,
                        "gdbarch_dump: IS_CALL_INSN = 0x%08lx\n",
                        (long) current_gdbarch->is_call_insn
                        /*IS_CALL_INSN ()*/);
#endif
...
int
gdbarch_is_call_insn_p (struct gdbarch *gdbarch)
{
  return gdbarch->is_call_insn != 0;
}

int
gdbarch_is_call_insn (struct gdbarch *gdbarch, CORE_ADDR pc)
{
  if (gdbarch->is_call_insn == 0)
    internal_error (__FILE__, __LINE__,
                    "gdbarch: gdbarch_is_call_insn invalid");
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_is_call_insn called\n");
  return gdbarch->is_call_insn (pc);
}

void
set_gdbarch_is_call_insn (struct gdbarch *gdbarch,
                          gdbarch_is_call_insn_ftype is_call_insn)
{
  gdbarch->is_call_insn = is_call_insn;
}

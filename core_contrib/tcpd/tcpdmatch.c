...
char  **argv;
{
	...
	if ((ai = find_inet_addr(server)) == 0)
	    exit(1);
	memset((char *) &server_sa, 0, sizeof(server_sa));
	request_set(&request, RQ_SERVER_SIN, &server_sa, 0);

	for (count = 0, ai2 = ai; ai2; count++, ai2 = ai2->ai_next) {
	    memcpy((char *) &server_sa, ai2->ai_addr, ai2->ai_addrlen);
     ...
     */
    if (dot_quad_addr(client) != INADDR_NONE || ipv6_addr(client, NULL)) {
     ...
     */
    if ((ai = find_inet_addr(client)) == 0)
	exit(1);
    memset((char *) &client_sa, 0, sizeof(client_sa));
    request_set(&request, RQ_CLIENT_SIN, &client_sa, 0);

    for (count = 0, ai2 = ai; ai2; count++, ai2 = ai2->ai_next) {
	memcpy((char *) &client_sa, ai2->ai_addr, ai2->ai_addrlen);

...
static char maskbits[] = { 0x00, 0x80, 0xC0, 0xE0, 0xF0, 0xF8, 0xFC, 0xFE };
...
char   *string;
{
     ...
     */
    if (ipv6_addr(net_tok, &net6)) {
	if (!ipv6_addr(string, &in6))
	    return (NO);
	if ((masklen = get_masklen(mask_tok, 128)) < 0) {
	...
	return (((*hp ^ *np) & maskbits[masklen]) == 0);
    }
    /*
     * Must be an IPv4 address, with a mask or a mask length.
    ...
    if ((mask = dot_quad_addr(mask_tok)) == INADDR_NONE) {
	/* The mask is not a dotted quad, see if it is a mask length */
	if ((masklen = get_masklen(mask_tok, 32)) < 0) {
	    tcpd_warn("bad net/mask expression: %s/%s", net_tok, mask_tok);
	    return (NO);				/* not tcpd_jump() */
	}

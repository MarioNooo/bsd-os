/*	BSDI boot1.c,v 1.5 2001/08/03 22:12:31 donn Exp	*/
...
#error MAP_ANONYMOUS not defined and suplementary value not known
    ...
    mmap_zero = malloc_buffer = (unsigned char *) _dl_mmap((void*) 0,
			     (size_t)pagesize,
...

  {
	...
	if(ppnt->p_type == PT_LOAD && !(ppnt->p_flags & PF_W))
	  _dl_mprotect((void *) (load_addr + (ppnt->p_vaddr & ~pagemask)),
    ...
    if(app_tpnt->dynamic_info[DT_TEXTREL]) {
      ...
      for(i=0; i<dl_data[AT_PHNUM]; i++, ppnt++) {
	if(ppnt->p_type == PT_LOAD && !(ppnt->p_flags & PF_W))
	  _dl_mprotect((void *) (ppnt->p_vaddr & ~pagemask),
	       (ppnt->p_vaddr & pagemask) + (unsigned int) ppnt->p_filesz,
	       PROT_READ | PROT_WRITE | PROT_EXEC);
      }
    }
...
/* Now we have done the mandatory linking of some things.  We are now
  ...
  for(tpnt = _dl_loaded_modules; tpnt; tpnt = tpnt->next)
	...
	_dl_mprotect((void *)(tpnt->loadaddr + (ppnt->p_vaddr & ~pagemask)),
	     (ppnt->p_vaddr & pagemask) + (unsigned int) ppnt->p_filesz,
  ...
  /* Make an effort to call leaf node initializers first.  */
  for(tpnt = _dl_loaded_modules; tpnt && tpnt->next; tpnt = tpnt->next)
    ;
  for(; tpnt; tpnt = tpnt->prev)
    {
...
void * _dl_malloc(int size) {
  ...
  if(_dl_malloc_addr-_dl_mmap_zero+size > _dl_pagesize) {
	_dl_mmap_zero = _dl_malloc_addr = (unsigned char *) _dl_mmap((void*) 0,
				size,
  				PROT_READ | PROT_WRITE, 

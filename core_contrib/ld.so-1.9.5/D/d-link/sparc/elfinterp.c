...
static char * _dl_reltypes[] = { "R_SPARC_NONE", "R_SPARC_8",
  ...
  "R_SPARC_10", "R_SPARC_11", "R_SPARC_64", "R_SPARC_OLO10",
  "R_SPARC_HH22", "R_SPARC_HM10", "R_SPARC_LM22",
  "R_SPARC_PC_HH22", "R_SPARC_PC_HM10", "R_SPARC_PC_LM22",
  "R_SPARC_WDISP16", "R_SPARC_WDISP19", "R_SPARC_GLOB_JMP",
...
int _dl_parse_relocation_information(struct elf_resolve * tpnt, int rel_addr,
  ...
  for(i=0; i< rel_size; i+= sizeof(struct elf32_rela), rpnt++){
    ...
    case R_SPARC_HI22:
      if (!symbol_addr)
        symbol_addr = (unsigned int) tpnt->loadaddr + rpnt->r_addend;
      else
...
#ifdef __bsdi__
    /*
     * N.B.: the compiler does not generate these under -fPIC, but we have
     * one special case in the C library, and it really IS okay to use these
     * in PIC.
     */
    case R_SPARC_WDISP22:
      diff = symbol_addr - (unsigned int) reloc_addr;
      diff >>= 2;
      /* if ((diff & 3) || ((diff >> 22) != 0 && (diff >> 22) != -1)) error */
      mask = (1 << 22) - 1;
      diff &= mask;
      *reloc_addr = (*reloc_addr & ~mask) | diff;
      break;
#endif
    case R_SPARC_COPY:

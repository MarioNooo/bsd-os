/*	BSDI hash.h,v 1.3 1998/09/09 06:11:04 torek Exp	*/
...
/* On m68k and sparc, constant strings are referenced through the GOT. */
...
#ifdef __bsdi__
/*
 * No symbols are fed in to this function for which it would return 0,
 * so we save some code space.
 */
#define _dl_symbol(name) 1
#else
extern inline int _dl_symbol(char * name)
{
#ifdef __sparc__	/* do resolve calls to .mul, .rem, etc. */
  if (name[0] == '.')
    return 1;
#endif

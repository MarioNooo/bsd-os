/* @(#)shs1drvr.c	12.3 9/1/96 11:44:16 */
/*
...
#define MAX_SHS1_TEST_DATA (sizeof(shs1_test_data)/sizeof(shs1_test_data[0]))
...
#define MAX_SHS1_TEST_FILE (sizeof(shs1_test_file)/sizeof(shs1_test_file[0]))
...
static void shs1Output P((char*, int, int, int, SHS1_INFO*));
static int shs1PreFileRead P((char*, BYTE**));
static void shs1TestSuite P((void));
static void shs1Help P((void));
 ...
 */
static void
shs1Output(str, quot, hexhdr, trail, dig)
 ...
 */
void
shs1Print(hexhdr, trail, shs1Info)
    int hexhdr;		/* 1 => print 0x before the hex value */
    int trail;		/* 1 => print a trailing ".0" */
    SHS1_INFO *shs1Info;
{
...
shs1TimeTrial()
{
    ...
    SHS1COUNT(&shs1Info, SHS1_READSIZE*TEST_CHUNKS);
    shs1Final(&shs1Info);
    ...
    char **argv;		/* the args */
{
	...
	case 'v':
	    printf("%s: version 3.%d.%s%s %s\n",
	        program, atoi(version)-10, "3",
	        (strcmp(shs1_what,"@(#)") == 0 &&
	         strcmp("@(#)","@(#)") == 0 &&
	         strcmp(shs1dual_what,"@(#)") == 0 &&
	         strcmp(SHS1_H_WHAT,"@(#)") == 0) ? "" : "+",
	        "96/09/01");
	    exit(0);
	case 'x':
	    x_flag = 1;
	...
	if (data_str != NULL) {
	    ...
	    SHS1COUNT(&digest, pre_str_len + data_str_len);

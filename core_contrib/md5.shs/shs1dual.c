/* @(#)shs1dual.c	12.3 17 Nov 1995 05:12:09 */
/*
...
/* static declarations */
static void multiData P((BYTE*, UINT, BYTE*, UINT, UINT, SHS1_INFO*));
static void multiStream P((BYTE*, UINT, FILE*, UINT, SHS1_INFO*));
static void multiFile P((BYTE*, UINT, char*, UINT, SHS1_INFO*));
static void multiOutput P((char*, int, int, UINT, SHS1_INFO*));
 ...
 */
static void
multiData(pre_str, pre_len, inString, in_len, cnt, digs)
    ...
    UINT cnt;			/* number of digests */
    SHS1_INFO *digs;		/* array of digests, cnt elements long */
{
 ...
 */
static void
multiStream(pre_str, pre_len, stream, cnt, digs)
    ...
    SHS1_INFO *digs;		/* array of digests, cnt elements long */
{
	...
	for (p=pre_str, pbeyond=pre_str+pre_len; p < pbeyond; ++p, ++indx) {
	    *(n[(indx = ((indx >= cnt) ? 0 : indx))]++) = *p;
		...
		SHS1COUNT(digs+indx, SHS1_BLOCKSIZE);
		buflen[indx] = 0;
		n[indx] = bufs[indx];
	    }
	}
 ...
 */
static void
multiFile(pre_str, pre_len, filename, cnt, digs)
    BYTE *pre_str;		/* string prefix or NULL */
    UINT pre_len;		/* length of pre_str */
    ...
    SHS1_INFO *digs;		/* array of digests, cnt elements long */
{
	...
	for (i=0; i < cnt; ++i) {
		...
		SHS1COUNT(digs+i, SHS1_CHUNKSIZE - digs[i].datalen);
	    }
	}
    }
...
/*
 * multiMail - main driver of shs1 dual routines
 */
void
multiMain(argc, argv, pre_str, pre_len, data_str, nospace, cnt)
    ...
    int nospace;		/* 1 => don't space seperate multi digests */
    UINT cnt;			/* number of digests to perform */
{

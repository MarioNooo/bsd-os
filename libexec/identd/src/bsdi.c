...
/*
** bsdi.c		Low level kernel access functions for BSDI
**
  ...
  char *what;
{
  ...
  {
    if (syslog_flag)
      syslog(LOG_INFO, "getbuf: kvm_read(%08x, %d) - %s : %m",
	     addr, len, what);
  ...
  int lport;
{
  ...
  do 
  {
    ...
    prev = current;
    current = pcbp->inp_next;
  } while (pcbp->inp_next != head &&
	   getbuf((long) pcbp->inp_next,
		  pcbp,
		  sizeof(struct inpcb),
		  "tcblist") &&
	   (struct inpcb *)(pcbp->inp_head) == head &&
	   pcbp->inp_prev == prev);

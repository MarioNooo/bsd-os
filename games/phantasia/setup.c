/*	BSDI	setup.c,v 2.3 1998/09/22 11:34:01 torek Exp	*/
	...
	char *argv[];
{
	...
	int ch;
	char *destdir=NULL;		/* prefix dir for files */
	char realfile[512];		/* path name buffer */

	while ((ch = getopt(argc, argv, "m:d:")) != EOF)
		switch(ch) {
		case 'm':
			monsterfile = optarg;
			break;
    ...
    srandom((unsigned) time((time_t *) NULL));	/* prime random numbers */
    ...
    if (destdir) {
	strcpy(realfile, destdir);
	strcat(realfile, _PATH_VOID);
    }
    else
	strcpy(realfile, _PATH_VOID);
    ...
    if (destdir) {
	strcpy(realfile, destdir);
	strcat(realfile, _PATH_MONST);
    }
    else
	strcpy(realfile, _PATH_MONST);

    if ((Monstfp = fopen(realfile, "w")) == NULL)
	Error("Cannot update %s.\n", realfile);
    else
	{
    ...
    if (destdir) {
	strcpy(realfile, destdir);
	strcat(realfile, _PATH_MOTD);
    }
    else
	strcpy(realfile, _PATH_MOTD);

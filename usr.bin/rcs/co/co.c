...
/*
 * co.c,v
 * Revision 2.3  1998/01/26 18:14:26  polk
 * Change behavior of -k<foo> flags when the file
 * is marked as "binary".  If the file is marked as binary, -k<foo> on
 * the command line will no longer override the binary notation in the
 * file.  I believe this should be true in the normal version of RCS, but
 * Paul doesn't agree.  This allows us to export all our sources with -kv
 * for the release without breaking files marked as binary (like perl5
 * modules and true binaries).
 *
 * Revision 2.2  1995/12/06 18:09:44  polk
...
mainProg(coId, "co", "co.c,v 2.3 1998/01/26 18:14:26 polk Exp")
{
...
#		endif
	...
	if (!Head) {
		...
		if (0 <= expmode && Expand != BINARY_EXPAND)
			Expand = expmode;

 ...
 *
 * $Log: perl.c,v $
 * Revision 1.4  1996/08/27 02:32:29  sanders
 * add /usr/libdata/perl/include to the standard @INC so we can symlink it
 * to the include files in /usr/libdata/perl5/site_perl/i386-bsdos/include
 * which hopefully will be slightly less broken than in current releases
 *
 * Revision 1.3  1996/06/26 23:31:38  sanders
 * suidperl security patch -- lwall's version plus BSDI fixes
 * (including adding the lseek() because the rewind() wasn't doing it)
 *
 * Revision 1.2  1993/12/22 17:08:26  polk
 * patch36
 *
 * Revision 1.1.1.1  1992/07/27  23:25:03  polk
...
#endif
    incpush(PRIVLIB);
    incpush("/usr/libdata/perl/include");	/* BSDI addition */
 ...
 %s | %s -C %s %s",
	...
	if (euid && stat(stab_val(curcmd->c_filestab)->str_ptr,&statbuf) >= 0 &&
	  statbuf.st_mode & (S_ISUID|S_ISGID)) {
	    (void)sprintf(buf, "%s/suidperl", BIN, patchlevel);
...
FIX YOUR KERNEL, PUT A C WRAPPER AROUND THIS SCRIPT, OR USE -u AND UNDUMP!\n");
    ...
    (void)sprintf(tokenbuf, "%s/taintperl", BIN, patchlevel);
	...
	(void)fclose(rsfp);
	(void)sprintf(buf, "%s/taintperl", BIN, patchlevel);
	execv(buf, origargv);	/* try again */

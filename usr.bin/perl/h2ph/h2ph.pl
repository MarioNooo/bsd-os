...
#	BSDI	h2ph.pl,v 1.2 1995/08/03 00:58:32 polk Exp

$perlincl = '/usr/libdata/perl';
...
sub expr {
	...
	s/^([_a-zA-Z]\w*)//	&& do {
	    ...
	    elsif ($isatype{$id}) {	# type: cast, decl, or subrarg
		if ($new =~ /{\s*$/) {	# probably decl
		    $new .= "$id";
		}
		elsif ($new =~ /\(\s*$/ && /^[\s*]*\)/) {	# cast
		    $new =~ s/\(\s*$//;

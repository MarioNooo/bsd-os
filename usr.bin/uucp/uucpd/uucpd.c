...
**
**	uucpd.c,v
**	Revision 2.3  1999/04/19 17:29:13  polk
**	remove bogus decl of inet_ntoa)
**	
**	Revision 2.2  1996/06/04 13:24:24  bostic
**	turn off echo in uucpd when reading the password, from Paul Vixie.
**	bug report #002778.
**	Note that this wasn't a bug and our changes didn't fix anything.  What's
**	happening is that the telnet client, which the user was using to test
**	uucpd, is running in line mode and therefore doing local echo.  uucpd is
**	not a telnet server and does not speak any of the telnet option
**	negotiation.  Therefore the passwords will echo.  Our changes will not
**	affect anything since tc_getattr() et al only work if you have a terminal.
**	We don't have a terminal, even a pseudo terminal, since we are just a
**	child of inetd with a TCP socket on stdin/stdout.  Our changes will hurt
**	nothing.  The user should be told "don't use telnet to talk to uucpd, if
**	echoed passwords bother you."  We are not going to teach uucpd how to
**	negotiate telnet options just for this kind of debugging stuff.
**	
**	Revision 2.1  1995/02/03 13:22:14  polk
**	Update all revs to 2.1
**
 * Revision 1.5  1995/01/08  07:49:04  donn
...
static char rcsid[]	= "uucpd.c,v 2.3 1999/04/19 17:29:13 polk Exp";
...
)
{
	...
	if ( passwd ) {
		if ( tcgetattr ( STDIN_FILENO, &t ) != -1 ) {
			...
			tt.c_lflag &= ~ECHO;
			tcsetattr(STDIN_FILENO, TCSANOW|TCSASOFT, &tt);
			set++;
		}
	}
	...
	if ( set ) {
		(void) tcsetattr(STDIN_FILENO, TCSANOW|TCSASOFT, &t);
	}
}
...
service()
{
	...
	for ( count = 0 ;; )
	{
		...
		if ( readline(username, sizeof(username), loginp, false) == SYSERROR )
		{
			...
			LOG(1, (lreaderr, hostname, strerror(errno)));
			continue;
		}

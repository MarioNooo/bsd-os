...
**
**	RCSID Args.c,v 2.2 2001/12/10 18:47:33 chrisk Exp
**
**	Args.c,v
**	Revision 2.2  2001/12/10 18:47:33  chrisk
**	Fix a buffer overflow from command line options. This could allow a
**	local user to get a shell with permissions of uucp user. Not that
**	that will really gain them much.
**	
**	Refer req. 79655.
**	
**	Revision 2.1  1995/02/03 13:16:29  polk
...
err:
		Trace((2, "\terror"));
		if ( (val = argusage(buf, sizeof(buf), up, count, argv0))
		    != NULLSTR )
			(void)(*usagerr)(val);
		errs++;
		goto out;
...
static char *
argusage(buf, bsize, up, argn, argp)
	char *		buf;
	size_t		bsize;
	register Args *	up;
	int		argn;
	char *		argp;
{
	...
	if ( up == (Args *)0 )
	{
		...
		(void)snprintf(buf, bsize, english("\"%s\" unexpected in %d%.2s argument"), argp, argn, cp);
		return buf;
	}
	...
	if ( FuncVal > ARGERROR )
	{
		if ( up->posn != OPTPOS || up->flagchar == NONFLAG || up->flagchar == MINFLAG )
			(void)snprintf(buf, bsize, english("argument %d %s \"%s\""), argn, FuncVal, argp);
		else
			(void)snprintf(buf, bsize, english("flag '%c' %s"), up->flagchar, FuncVal);
		return buf;
	}
	...
	if ( up->posn != OPTPOS )
		(void)snprintf(buf, bsize, english("argument %d must be %s <%s>"), argn, np, cp);
	else
	if ( up->flagchar == NONFLAG )
		(void)snprintf(buf, bsize, english("expected %s <%s> at argument %d"), np, cp, argn);
	else
	if ( up->flagchar == MINFLAG )
		...
		(void)snprintf(buf, bsize, english("flag '%c' needs %s <%s>"), up->flagchar, np, cp);

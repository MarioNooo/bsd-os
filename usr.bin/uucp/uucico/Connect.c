...
**
**	RCSID Connect.c,v 2.2 1996/06/11 19:27:24 vixie Exp
**
**	Connect.c,v
**	Revision 2.2  1996/06/11 19:27:24  vixie
**	2.2
**	
**	Revision 2.1  1995/02/03 13:21:09  polk
**	Update all revs to 2.1
**
 * Revision 1.4  1995/01/08  07:48:42  donn
...
CloseACU()
{
	...
	*/
	...
	if ( Is_a_tty && ioctl(Dcf, TIOCNXCL, (void *)0) == SYSERROR )
		Debug((5, "CloseACU ioctl %s", strerror(errno)));
#endif
...
)
{
	...
	*cp = '\0';
	...
	else
	{
		...
		while ( (cp = GetLine(&bufp, false)) != NULLSTR )
		{
...
)
{
	...
	if ( fcntl(fd, F_SETFD, 1) == SYSERROR )
		Debug((2, CouldNot, "F_SETFD", strerror(errno)));
#endif	/* F_SETFD */
}
...
)
{
	...
	for (tc = TermioCtl;  tc->prefix != NULL;  tc++) {
		...
		if (strcmp(str+len, "_ON") == STREQUAL)
			...
			continue;
		Debug((5, "TERMIO(%s,%d)", tc->prefix, onoff));
...
)
{
	...
	if ( write(fd, (char *)&c, 1) != 1 )
	{
		...
		Log("BAD WRITE %s: %s", ttyn, strerror(errno));

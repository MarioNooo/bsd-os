...
**
**	RCSID Diallers.c,v 2.2 1996/06/11 19:27:25 vixie Exp
**
**	Diallers.c,v
**	Revision 2.2  1996/06/11 19:27:25  vixie
**	2.2
**	
**	Revision 2.1  1995/02/03 13:21:14  polk
**	Update all revs to 2.1
**
 * Revision 1.7  1995/01/08  07:48:49  donn
...
#endif	/* BSDTCP */
#ifdef PROG
	{ "PROG", "program", progopn, nullopn, progcls },
#endif	/* PROG */
#ifdef MICOM
...
)
{
	...
	if (dcr < 0) {
		...
		Debug((4, "OPEN FAILED: %s", strerror(errno)));
		rmlock(dev.D_line);
		return (CF_DIAL);
	}
...
)
{
	...
	if ( (cp = GetLine(bufp, false)) == NULLSTR )
		return false;
...
)
{
	...
	if (ioctl(tty, TIOCHPCL, (void *)0) == SYSERROR)
		Debug((1, CouldNot, "TIOCHPCL", strerror(errno)));

#endif	/* USE_TERMIOS */

#ifdef TIOCEXCL
	if (ioctl(tty, TIOCEXCL, (void *)0) == SYSERROR)
		Debug((1, CouldNot, "TIOCEXCL", strerror(errno)));
#endif
...
)
{
	...
	if (tcsendbreak(fn, 0) == SYSERROR)
		Debug((5, "tcsendbreak %s", strerror(errno)));
	...
	if ( ioctl(fn, TIOCSBRK, (void *)0) == SYSERROR )
		Debug((5, "TIOCSBRK %s", strerror(errno)));

	UUDELAY(null_count, 10);

	if ( ioctl(fn, TIOCCBRK, (void *)0) == SYSERROR )
		Debug((5, "TIOCCBRK %s", strerror(errno)));
...
badbreak:	(void)alarm(0);
		Log("BAD WRITE for BRK %s", strerror(errno));
		longjmp(AlarmJmp, 3);
	}

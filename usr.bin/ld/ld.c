/*	BSDI ld.c,v 2.3 1997/04/21 00:09:47 torek Exp	*/
...
#ifdef sparc
#ifndef sun	/* a BSD environment; turn on sparc-specific code */
#define sun 1
/* Perhaps we should make M_SPARC and a_machtype #define's for this... */
...
#define RELOC_PCREL_P(r)		(reloc_target_pcrel_p[(r)->r_type])
...
/* Note that these are very dependent on the order of the enums in
   ...
   changed */
/* Also note that SFA_BASE..RELATIVE may be incorrect; I have no information */
static int reloc_target_pcrel_p[] = {
  0, 0, 0,			/* RELOC_8 through RELOC_32 */
  1, 1, 1,			/* RELOC_DISP8 through RELOC_DISP32 */
  1, 1,				/* RELOC_WDISP30, RELOC_WDISP22 */
  0, 0, 0, 0,			/* RELOC_HI22, RELOC_22, RELOC_13, RELOC_LO10 */
  0,				/* RELOC_SFA_BASE */
  0,				/* RELOC_SFA_OFF13 */
  0,				/* RELOC_BASE10 */
  0,				/* RELOC_BASE13 */
...
static int reloc_target_rightshift[] = {
  0, 0, 0, 0, 0, 0,		/* RELOC_8 through RELOC_DISP32 */
  2, 2, 10, 0, 0, 0,		/* RELOC_WDISP30 through RELOC_LO10 */
  0,				/* RELOC_SFA_BASE */
  0,				/* RELOC_SFA_OFF13 */
  0,				/* RELOC_BASE10 */
  0,				/* RELOC_BASE13 */
...
static int reloc_target_size[] = {
  0, 1, 2, 0, 1, 2,		/* RELOC_8 through RELOC_DISP32 */
  2, 2, 2, 2, 2, 2,		/* RELOC_WDISP30 through RELOC_LO10 */
  2,				/* RELOC_SFA_BASE */
  2,				/* RELOC_SFA_OFF13 */
  2,				/* RELOC_BASE10 */
  2,				/* RELOC_BASE13 */
...
static int reloc_target_bitsize[] = {
  8, 16, 32, 8, 16, 32,		/* RELOC_8 through RELOC_DISP32 */
  30, 22, 22, 22, 13, 10,	/* RELOC_WDISP30 through RELOC_LO10 */
  32, 				/* RELOC_SFA_BASE */
  32,				/* RELOC_SFA_OFF13 */
  16,				/* RELOC_BASE10 */
  0,				/* RELOC_BASE13 */
  ...
  16,				/* RELOC_WDISP2_14 */
  19,				/* RELOC_WDISP19 */
  0,				/* RELOC_HHI22 -- a.out only has 32 bits */
  0,				/* RELOC_HLO10 -- a.out only has 32 bits */
};
...
digest_symbols ()
{
  ...
  if (!relocatable_output)
	...
	= (2 * set_symbol_count + set_vector_count) * sizeof (unsigned long);
      /* However, this must be rounded up as required by the machine's
	 alignment constraints (MAX_ALIGNMENT). */
      set_sect_size
	= (set_sect_size + MAX_ALIGNMENT - 1) & ~(MAX_ALIGNMENT - 1);
      ...
      for (sp = symtab[i]; sp; sp = sp->link)
	{
	  ...
	  for (p = sp->refs; p; p = next)
	      ...
	      if (SET_ELEMENT_P (type))
		{
	      ...
	      else if ((type & N_EXT) && type != (N_FN | N_EXT) &&
	               type != (N_UNDF | N_EXT))
		{
...
write_output ()
{
  ...
  /* only remove regular files, so, e.g., "ld -o /dev/null" is safe */
  if (stat (output_filename, &statbuf) >= 0 && S_ISREG (statbuf.st_mode))
...
#if defined(sun) && (TARGET == SUN4)
	case 214:		/* XXX ugly special V9 case */
#define SHUFFLE(x) ((((x) & (3 << 14)) << (20 - 14)) | ((x) & ~(3 << 14)))
	  mask = SHUFFLE(mask);
	  relocation = SHUFFLE(relocation);
#undef SHUFFLE
	  /* FALLTHROUGH */
#endif

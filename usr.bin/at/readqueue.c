 ...
 *
 *	BSDI readqueue.c,v 2.4 1997/11/19 01:58:34 sanders Exp
 */
...
readqueue()
{
	...
	while ((dp = readdir(dirp)) != NULL) {
		...
		p->ngroups = 0;
		while (s && *s) {
			p->groups[p->ngroups++] = atol(s);
			Pmsg(("group found: %d %ld\n",
				p->ngroups-1, atol(s)));
			s = index(s+1, ' ');
		}
...
sortqueue(JobQ head)
{
	...
	for (p = head->next; p; (count++, (p = p->next)))
		Pmsg(("found: 0x%08x -> 0x%08x\n", p, p->next));
	Pmsg(("counted %d\n", count));
	if (count > 0) {
...
printitem(register JobQ p)
{
	...
	printf("%10d %10.10s %10s    ",
		p->id, user_from_uid(p->owner, 0), s);
	print_time(p->when);
	printf("\n");
}

 ...
 *
 *	BSDI dateyacc.y,v 2.4 1999/02/15 17:12:43 prb Exp
 */
...
DEF_ADDUNIT(1,dpm[isleap(time_v.tm_year+1900)][time_v.tm_mon],mday,mon)
...
do_time(int i, int fmt)
{
	if (fmt == FMT_NOON && i != 12*100)
		Pexit("noon is 12:00n not %02d:%02d\n", i/100, i%100);
	if (fmt == FMT_MIDNIGHT && i != 12*100)
		Pexit("midnight is 12:00m not %02d:%02d\n", i/100, i%100);
	if (fmt == FMT_MIDNIGHT)
		i = 0;
	...
	if (fmt == FMT_AM || fmt == FMT_PM) {
		if (time_v.tm_hour > 12)
			Pexit("hours field is invalid for a 12-hour clock\n");
		if (time_v.tm_hour < 1)
			Pexit("hours field is invalid for a 12-hour clock\n");

		/* am/pm change at the meridian so we compensate here */
		if (time_v.tm_hour==12)
			time_v.tm_hour = 0;
	}
	if (fmt == FMT_PM)
...
do_hrmin(char *s)
{
	...
	Pmsg(("do_hrmin got ``%s''\n", s));
	if (len > 4)
		Pexit("invalid time specification ``%s''\n", s);
	if (len > 2) {
	...
	if (hr > 23)
		Pexit("hours field is invalid for a 24-hour clock\n");
	if (min > 59)
		Pexit("minutes field is invalid\n");
	return hr * 100 + min;
}
...
do_date()
{
	int x = dpm[isleap(time_v.tm_year+1900)][time_v.tm_mon];
	if (time_v.tm_mday > x)
		Pexit("only %d days in month\n", x);
}
...
do_incr(int i, char *s)
{
	...
	Pexit("increment %d %s is not valid\n", i, s);
}
...
adjustday()
{
	...
	time_v.tm_isdst = -1;
	delta = mktime(&time_v) - mktime(&lc_time);

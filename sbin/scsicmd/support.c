...
static const char rcsid[] =
    "support.c,v 2.9 2000/01/05 22:51:46 polk Exp";
	...
	int flags;
{
	...
	if (bftab[1].bf_name) {
		buf = bftab[1].bf_buf;
		if (bftab[1].bf_len > size)
			size = bftab[1].bf_len;
	} else if ((buf = malloc_buf = malloc(size)) == 0)
		err(1, "%s: can't allocate dummy buffer", cmdp->cmd_name);

	suc.suc_flags = flags;
	suc.suc_cdblen = bftab[0].bf_len;
	bcopy(bftab[0].bf_buf, suc.suc_cdb, bftab[0].bf_len);
	suc.suc_datalen = size;
	suc.suc_data = (caddr_t)buf;
	suc.suc_timeout = 0;	/* forever */

	if (ioctl(scsi, SCSIRAWCDB, &suc) == -1)
		err(1, "submit of SCSI command to %s", scsiname);

	if (suc.suc_sus.sus_status != 0)
		scsi_sense(&suc.suc_sus);
	if (malloc_buf)
		free(malloc_buf);
}
...
read_var_cmd()
{
	...
	bftab[1].bf_len = pmp->pm_byte + pmp->pm_value +
	    howmany(pmp->pm_len, NBBY);
}
...
read_data_cmd()
{
	...
	pmp = extract_bf("tl", 0);
	if (pmp->pm_value == 0) {
		warnx("%s: zero-length read", cmdp->cmd_name);
		if ((buf = malloc(BUFFER_SIZE)) == 0)	/* XXX */
			err(1, "%s: can't allocate dummy buffer", cmdp->cmd_name);
		len = BUFFER_SIZE;
	} else if ((buf = malloc(pmp->pm_value)) == 0)
		err(1, "%s: can't allocate data buffer", cmdp->cmd_name);
	else
		len = pmp->pm_value;
...
mode_select_cmd()
{
	...
	save_default("mdl", &mdl);
	save_default("mt", &mt);

	sprintf(parmlist, "code=0x15,ps=0,pf=%d,length=%d",
	    page_code ? 1 : 0, mdl + 1);
	set_parameters(parmlist);

	scsi_cmd(SUC_WRITE);

	restore_default("mdl", &mdl);
}

/*
 * The kernel automatically performs a REQUEST SENSE after
 * a command fails, so we just grab the most recent REQUEST SENSE data.
 */
void
request_sense_cmd()
{
	static scsi_user_sense_t s;
	bf *bftab = current_cmd->cmd_bf;

	if (ioctl(scsi, SCSIGETSENSE, &s) == -1) {
		warn("ioctl(SCSIGETSENSE) failed on %s", scsiname);
		return;
	}
	bcopy(s.sus_sense, bftab[1].bf_buf, sizeof s.sus_sense);
}
...
void
scsi_sense(sus)
	struct scsi_user_sense *sus;
{
	struct scsi_sense *sn = (struct scsi_sense *)sus->sus_sense;
	char *p;
	int key;

	fprintf(stderr, "scsi status: ");
	if (p = (char *)scsistatusmsg[(sus->sus_status & STS_MASK) >> 1])
		fputs(p, stderr);
	else
		fprintf(stderr, "%#x", sus->sus_status);
	if (sus->sus_status & STS_CHECKCOND) {
		if (SENSE_ISXSENSE(sn) && XSENSE_ISSTD(sn)) {
			...
			if (XSENSE_KEY(sn) == SKEY_ILLEGAL &&
			    XSENSE_ADDL(sn) >= 10 && sn->sn_addl[7] & 0x80) {
				fprintf(stderr, ", field %d",
				    sn->sn_addl[8] << 8 | sn->sn_addl[9]);
				if (sn->sn_addl[7] & 8)

 ...
 */
/*	BSDI partition.cc,v 2.6 1998/08/31 20:43:58 prb Exp	*/
...
void
Partition::Print(int y, quad_t gap)
{   
    move(y,0); clrtoeol();
    
    mvprint(y, 0, "%c%c | %02X %-10.10s |%10qd (%9.1f %9.1f)|%10qd |%10qd | ",
        number + '0',
        number == disk.active ? '*' : ' ',
        type,
        PTypeFind(type).name,   
...
PType ptypes[] = {
        ...
        { "DOS-FAT32", 		0x0B,		force_track, 1, 1, },
        { "VENIX", 		0x40,		force_track, 1, 1, },
        { "PPC-BOOT", 		0x41,		force_sector, 0, 1, },

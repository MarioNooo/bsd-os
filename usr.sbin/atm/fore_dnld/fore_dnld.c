 ...
 * All rights reserved.
 *
 *	@(#) fore_dnld.c,v 1.15 1998/08/26 23:29:32 mks Exp
 *
 */
...
#ifndef lint
static char *RCSid = "@(#) fore_dnld.c,v 1.15 1998/08/26 23:29:32 mks Exp";
#endif
...
#include <dev/atm/fore/fore.h>
#include <dev/atm/fore/fore_aali.h>
#include <dev/atm/fore/fore_slave.h>
...
#if (defined(BSD) && (BSD >= 199103))
#ifdef	__bsdi__
#define	DEV_NAME _PATH_DEV FORE_DEV_NAME "%d"
#else
#define	DEV_NAME "/dev/kmem"
#endif
#endif	/* BSD */

#if	defined(sun) && !defined(BYTE_ORDER)
...
void *ram;
{
	...
	if ( verbose )
		printf("loading %.4s %#x start %#x entry %#x\n",
		    (char *)&binhdr.Id, binhdr.fver, binhdr.start,
		    binhdr.entry);
	 ...
	 */
	{
		...
		getline ( verbose );	/* this is requied */
...
char *argv[];
{
	...
	for (; req.air_buf_len >= sizeof(struct air_cfg_rsp); 
		...
		if (air->acp_vendor == VENDOR_FORE )
		{
			 ...
			 */
#ifdef	__bsdi__
			sprintf ( bus_dev, DEV_NAME, air->acp_unit );
#else
			...
			ram = mmap(0, PCA200E_MMAP_SIZE,
#else
			ram = mmap(0, air->acp_ramsize,
#endif
			...
			Mon = (Mon960 *)((u_char *)ram + MON960_BASE);
			...
			if ( comm_mode ) {
			     ...
			     */
			     aap = (Aali *)((u_char *)ram + CP_READ(Mon->mon_appl));
				...
				if ( verbose )
					fprintf(stderr, "%s heartbeat %x %x\n",
					    air->acp_intf, hb1, hb2);
				if (hb1 < hb2)
					break;
			     }
			}

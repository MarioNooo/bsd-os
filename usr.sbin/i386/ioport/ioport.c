 ...
 *
 *	ioport.c,v 2.5 2000/09/28 03:59:48 donn Exp
 */
	...
	struct iorange *rp;
{
	...
	static struct nlist nl[] = {
		/* Currently all TSSes contain identical IOPL maps.  */
		{ "first_pcpu" },
#define	X_PCPU	0
		{ NULL }
	};
	...
	if (i > 0) {
		for (i = 0; i < sizeof nl / sizeof nl[0] - 1; i++)
			if (nl[i].n_value == 0)
				warnx("kvm_nlist: symbol %s not found",
				    nl[i].n_name);
		exit(1);
	}
	tss_kva = (u_long)nl[X_PCPU].n_value + offsetof(struct pcpu, tss);
	if (kvm_read(kd, tss_kva, (char *)&tss, sizeof tss) != sizeof(tss))

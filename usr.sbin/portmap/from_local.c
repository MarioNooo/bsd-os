/*      BSDI from_local.c,v 2.4 2000/02/23 16:49:27 dab Exp        */
 /*
...
find_local()
{
    ...
    /* 
     * Build up an array of pointers to just the active IP interfaces.
     */
    for (num_local = 0; ifap != NULL; ifap = ifap->ifa_next) {
	/* XXX what about AF_INET6 ?  */
	if (ifap->ifa_addr->sa_family == AF_INET && ifap->ifa_flags & IFF_UP) {
	    if (num_local >= num_addrs && grow_addrs() == 0)
		    break;
	    addrs[num_local++] = ((struct sockaddr_in *)
				  ifap->ifa_addr)->sin_addr;
	}
    }
#else
    struct ifconf ifc;

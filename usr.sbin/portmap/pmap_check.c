/*      BSDI pmap_check.c,v 2.4 1999/04/19 17:34:01 polk Exp        */
 /*
...
int
check_setunset(xprt, ludp_xprt, ltcp_xprt, proc, prog, check_port, port)
SVCXPRT *xprt;
SVCXPRT *ludp_xprt;
SVCXPRT *ltcp_xprt;
u_long  proc;
...
u_long  port;
{
    ...
    if (check_port && !check_privileged_port(addr, proc, prog, port))
	return (FALSE);
    if (verboselog)
	log_client(addr, proc, prog);
    return (TRUE);
...
int
check_setunset(addr, proc, prog, check_port, port)
struct sockaddr_in *addr;
u_long  proc;
u_long  prog;
bool_t	check_port;

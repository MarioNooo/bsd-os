/*	BSDI malloc.c,v 2.10 1998/05/26 22:12:12 donn Exp	*/
 ...
 * ----------------------------------------------------------------------------
 *
 * malloc.c,v 2.10 1998/05/26 22:12:12 donn Exp
 *
 */
...
#   define THREAD_LOCAL int __needlock
#   define THREAD_LOCK()						\
	if (__needlock = !_thread_kern_locked())			\
	    _thread_kern_enter();
...
/* If we should display any run-time warnings. */
...
/*
 * Don't screw up users who replace malloc()/free() and
 * expect that calloc() will call their malloc().
 * We leave an __calloc() entry point in case someone wants it.
 */
void *
__calloc(size_t num, size_t size)
{
    register void *r;
    THREAD_LOCAL;

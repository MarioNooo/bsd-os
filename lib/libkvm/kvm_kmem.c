 ...
 *
 *	BSDI kvm_kmem.c,v 2.2 1997/05/25 15:09:01 jch Exp
 */
    ...
    int ns)
{
	...
	if ((addr = _kvm_symbol(kd, "_bucket")) == 0 ||
	    kvm_read(kd, addr, bp, nb * sizeof *bp) != (ssize_t)(nb * sizeof *bp))
		return (-1);
	if ((addr = _kvm_symbol(kd, "_kmemstats")) == 0 ||
	    kvm_read(kd, addr, sp, ns * sizeof *sp) != (ssize_t)(ns * sizeof *sp))
		return (-1);
	return (0);
}

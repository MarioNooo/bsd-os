 ...
 * Modified 1994, 1995 by Chris Torek at BSDI.
 *
 *	BSDI sparc.c,v 2.4 1995/10/05 03:44:16 torek Exp
 */
...
static void s_seg(), s_proc(), s_data1(), s_reserve(), s_common(), s_ignore();
...
/*
 * Ignore a directive -- this is a hack!
 */
static void
s_ignore()
{
...
s_reserve()
{
    ...
    *p = c;
    ...
    input_line_pointer ++;
    if ((bssize = get_absolute_expression()) < 0) {
	as_bad("BSS length (%d.) <0! Ignored.", bssize);
	ignore_rest_of_line();
	return;
    }
    *p = 0;
    ...
    *p = c;
    ...
    else if (strncmp(input_line_pointer, ",\".bss\"", 7) == 0)
	...
	align = align_log2(get_absolute_expression());
	if (align < 0) {
	    ignore_rest_of_line();
	    return;
	}
    } else
	align = 0;		/* no alignment (ie, byte align) */
	...
	&& symbolP->sy_value == 0))) {
	    /*
	     * This is not optimal -- the alignment really should be
	     * done by re-ordering the symbols, rather than just leaving
	     * a hole.
	     */
	    addr = local_bss_counter;
	    mask = ~((~0) << align);
	    addr = (addr + mask) & ~mask;
	    symbolP->sy_value = addr;
	    symbolP->sy_type  = N_BSS;
	    symbolP->sy_frag  = & bss_address_frag;
	    local_bss_counter = addr + bssize;
    } else {
    ...
    char *str;
{
	...
	*--s = ',';
	...
	for (args = insn->args; ; ++args) {
	    ...
	    case 'L':   /* 30 bit PC relative immediate */
